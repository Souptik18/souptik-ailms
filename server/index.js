...
import { getCourseCatalogCategories, queryCourseCatalog } from './courseCatalogService.js'
import { getLinkedInJobs } from './jobsService.js'
import { generateOnboardingRoadmap, OnboardingAiProviderError } from './onboardingRoadmapService.js'
...
app.get('/api/course-catalog', (request, response) => {
  try {
    const rawKeywords = String(request.query?.keywords ?? '')
    const payload = queryCourseCatalog({
      page: Number.parseInt(String(request.query?.page ?? '1'), 10),
      pageSize: Number.parseInt(String(request.query?.pageSize ?? '10'), 10),
      search: request.query?.search ?? '',
      access: request.query?.access ?? 'all',
      category: request.query?.category ?? 'all',
      sort: request.query?.sort ?? 'recommended',
      recommendedOnly: String(request.query?.recommendedOnly ?? 'false').toLowerCase() === 'true',
    ...
    response.json({
      ...payload,
      categories: getCourseCatalogCategories(),
    })
  } catch {
    response.status(500).json({ error: 'Unable to load course catalog.' })
  }
})
...
app.get('/api/jobs/linkedin', async (request, response) => {
  try {
    const payload = await getLinkedInJobs({
...
app.post('/api/onboarding/roadmap', async (request, response) => {
  try {
    const result = await generateOnboardingRoadmap({
      stage: request.body?.stage,
      role: request.body?.role,
    })
    response.json(result)
  } catch (error) {
    const status = error instanceof OnboardingAiProviderError ? error.status : 400
    response.status(status).json({
      error: error instanceof Error ? error.message : 'Unable to generate onboarding roadmap.',
      provider: error instanceof OnboardingAiProviderError ? error.provider : undefined,
    })
  }
})

...
function PublicSubjectView({
  ...
  const [visibleVideoCount, setVisibleVideoCount] = useState(CATALOG_VIDEO_LIMIT)
  ...
  useEffect(() => {
    ...
    setVisibleVideoCount(CATALOG_VIDEO_LIMIT)
    setNextPageToken('')
  ...
  const cappedVisibleCount = Math.min(visibleVideoCount, CATALOG_VIDEO_LIMIT)
  const cappedVideoTotal = Math.min(videos.length, CATALOG_VIDEO_LIMIT)
  const visibleVideos = videos.slice(0, cappedVisibleCount)
  const hasMoreVideos = cappedVisibleCount < cappedVideoTotal || (videos.length < CATALOG_VIDEO_LIMIT && Boolean(nextPageToken))
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) {
        if (cappedVisibleCount < cappedVideoTotal) {
          setVisibleVideoCount((currentCount) => Math.min(currentCount + CATALOG_VIDEO_LIMIT, cappedVideoTotal))
          return
        }

        if (!nextPageToken || isLoadingMoreVideos || videos.length >= CATALOG_VIDEO_LIMIT) return
        ...
        fetch(buildApiUrl(`/api/lms-videos?category=${encodeURIComponent(activeSubject)}&subcategory=${encodeURIComponent(activeSubcategory)}${filterParam}&pageToken=${encodeURIComponent(nextPageToken)}`))
          ...
          .then((payload) => {
            ...
            setNextPageToken(typeof payload?.nextPageToken === 'string' ? payload.nextPageToken : '')
            setVisibleVideoCount((currentCount) => Math.min(currentCount + Math.max(1, incomingVideos.length), CATALOG_VIDEO_LIMIT))
          })
          .catch(() => {
            setNextPageToken('')
          })
  ...
  }, [activeFilterKey, activeSubject, activeSubcategory, cappedVideoTotal, cappedVisibleCount, hasMoreVideos, isLoadingMoreVideos, mode, nextPageToken, previewVideo, videos.length])

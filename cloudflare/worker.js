...
function buildCookie(value, { maxAge = SESSION_INACTIVITY_LIMIT_MS, sameSite = 'Lax', secure = true } = {}) {
  const parts = [
...
function buildClearCookie({ sameSite = 'Lax', secure = true } = {}) {
  const parts = [
    `${SESSION_COOKIE_NAME}=`,
    'Path=/',
    'HttpOnly',
...
async function verifyFirebaseIdToken(env, idToken) {
  ...
  if (env.FIREBASE_PROJECT_ID && aud !== String(env.FIREBASE_PROJECT_ID)) {
...
async function runOnboardingRoadmapCompletion(env, stage, role) {
  ...
  try {
    ...
    throw new Error(`${provider.name} is not reachable from the Worker.`)
  }
  if (!response.ok) {
    const message = response.status === 401 || response.status === 403
      ? `${provider.name} rejected the configured API key.`
      : `${provider.name} roadmap generation failed with HTTP ${response.status}.`
  ...
  try {
    ...
    throw new Error(`${provider.name} returned an invalid roadmap response.`)
  }
}
...
async function handleRequest(request, env) {
  ...
  if (pathname === '/api/session/login' && request.method === 'POST') {
    ...
    const idToken = sanitizeText(body?.idToken ?? '', 2000)
